...
const App = () => {
  ...
  const circle: CircleRegion = useMemo(() => {
    return {
  ...
  const isUserInCircle = useCallback(
  ...
  useEffect(() => {
    ...
    if (position) {
      const nowInside = isUserInCircle(position, circle);
  ...
  }, [circle, isInside, isUserInCircle, position]);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={{ rowGap: 20, marginBottom: 30 }}>
        ...
        <View style={styles.buttonContainer}>
          <Button title="Stop watching" onPress={clearWatch} color={'white'} />
        </View>
      </View>
    </View>
  );
